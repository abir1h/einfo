...
class _LandingScreenState extends State<LandingScreen> with WidgetsBindingObserver {
  ...
  Future<bool> _handleWebViewFileUploadPermissions() async {
    if (Platform.isAndroid) {
      ...
      final needsSettings = results.values.any((status) => status.isPermanentlyDenied);
                ...
                Expanded(
                  child: hasInternet
                    ...
                    shouldOverrideUrlLoading: (controller, navAction) async {
                      ...
                      final isLogOutSuccess = uri.host.contains("einfo.site") &&
                          uri.pathSegments.length > 1 &&
                      ...
                      if (isLogOutSuccess) {
                        ...
                        try {
                          final fcmToken = await FirebaseMessaging.instance.getToken();
                          if (fcmToken != null) {
                            await SendTokenGateway.removeToken(fcmToken, username);
                          }
                        } catch (e) {
                          debugPrint("❌ Failed to get FCM token: $e");
                        }
                      ...
                      final isInternal = uri.host.contains("einfo.site") || uri.host.contains("einfosite.com");
                      if (!isInternal) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text("No Internet Connection", style: TextStyle(fontSize: 18, color: textColor.withOpacity(0.7))),
          const SizedBox(height: 10),
          Text("Waiting for internet...", style: TextStyle(color: textColor.withOpacity(0.5))),
        ],
      ),
    );
  }
